...
const { itIf, isBasicToken, isMintableBurnable } = require('../helpers')
    ...
    Object.entries(tokenContracts).forEach(([name, tokenProps]) => {
      ...
      describe(`Token creation functionality for ${name}`, () => {
        ...
        const createBasicToken = async (...args) => {
          const { tokenAddress } = await createTokenWrapper(createToken, ...args)
        ...
        describe('Create token validations', () => {
          ...
          itIf(isBasicToken(name), 'cannot create token with 0 supply', async () => {
            await truffleAssert.fails(
          ...
          itIf(isMintableBurnable(name), 'can create token with 0 supply', async () => {
